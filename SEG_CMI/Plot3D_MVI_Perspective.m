...
xx = (xn(2:end) + xn(1:end-1))/2 - min(dx)/2;   xx = xx(padW+1:end-padE);
yy = (yn(2:end) + yn(1:end-1))/2 - min(dy)/2;   yy = yy(padS+1:end-padN);
zz = (zn(2:end) + zn(1:end-1))/2 - min(dz)/2;   zz = zz(padT+1:end-padB);
...
[YY2D,ZZ2D] = meshgrid(min(yy):40:max(yy),min(zz):40:max(zz));
