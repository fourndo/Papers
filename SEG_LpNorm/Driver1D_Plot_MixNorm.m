...
ylabel('$\mathbf{f}(x)$', 'interpreter', 'latex','FontSize',16)
...
set(figure, 'Position', [50 0 775 400]);
axs{1} = axes('Position',[-.1125 .175 .8 .8]);
p = [0 0.5 1 1.5 2];
epsl = 1e-2;
xx=-1:1e-5:1;
for pp = 1 : length(p)
...
% pvec, qvec and lvec.
axs{2} = axes('Position',[0.39 .175 .8 .8]);
for rr = 4
...
%             axis square
        ...
        if rr < 4
            ...
            text(.15,0.-0.035,['$\mathbf{p: ' num2str(LP(1,1)) ',\; q: ' num2str(LP(1,2)) '}$'],'interpreter', 'latex','FontSize',12)
            text(.625,0.-0.035,['$\mathbf{p: ' num2str(0) ',\; q: ' num2str(LP(2,2)) '}$'],'interpreter', 'latex','FontSize',12,'BackgroundColor','w')
            quiver(0.4,-0.035,0.1,0, 'LineWidth', 2, 'MaxHeadSize',0.5, 'Color','k');
            quiver(0.1,-0.035,-0.1,0, 'LineWidth', 2, 'MaxHeadSize',0.5, 'Color','k');
            quiver(0.9,-0.035,0.1,0, 'LineWidth', 2, 'MaxHeadSize',0.5, 'Color','k');
            quiver(0.6,-0.035,-0.1,0, 'LineWidth', 2, 'MaxHeadSize',0.5, 'Color','k');
...
%             set(gca,'YTickLabel',[]);
            ...
            set(get(gca,'YLabel'),'Rotation',360);
            ylabh = get(gca,'YLabel');
            set(ylabh,'Position',get(ylabh,'Position') - [0.075 0 0]);
